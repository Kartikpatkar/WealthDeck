...
export async function seedDefaultCategories() {
  const defaults = [
    ...
    { name: 'Bill', type: 'expense', icon: '<svg class="svg-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"></path><polyline points="14 2 14 8 20 8"></polyline><line x1="16" y1="13" x2="8" y2="13"></line><line x1="16" y1="17" x2="8" y2="17"></line><polyline points="10 9 9 9 8 9"></polyline></svg>', color: '#a78bfa' },
    { name: 'Family', type: 'expense', icon: '<svg class="svg-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"></path><circle cx="9" cy="7" r="4"></circle><path d="M23 21v-2a4 4 0 0 0-3-3.87"></path><path d="M16 3.13a4 4 0 0 1 0 7.75"></path></svg>', color: '#fbbf24' },
    ...
    // Transfer (4)
    { name: 'Account Transfer', type: 'transfer', icon: '<svg class="svg-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><path d="M8 3 4 7l4 4"/><path d="M4 7h16"/><path d="m16 21 4-4-4-4"/><path d="M20 17H4"/></svg>', color: '#9ca3af' },
    { name: 'Credit Card Payment', type: 'transfer', icon: '<svg class="svg-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><rect x="1" y="4" width="22" height="16" rx="2" ry="2"/><line x1="1" y1="10" x2="23" y2="10"/></svg>', color: '#6366f1' },
    { name: 'Investment Transfer', type: 'transfer', icon: '<svg class="svg-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><polyline points="23 6 13.5 15.5 8.5 10.5 1 18"/><polyline points="17 6 23 6 23 12"/></svg>', color: '#10b981' },
    { name: 'Cash Withdrawal', type: 'transfer', icon: '<svg class="svg-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><line x1="12" y1="1" x2="12" y2="23"/><path d="M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6"/></svg>', color: '#f59e0b' }
  ];
...
export async function deleteCategory(id) {
  const db = getDB();
  return new Promise((resolve, reject) => {
    const transaction = db.transaction(['categories', 'transactions', 'budgets', 'bills'], 'readwrite');
    ...
    // Un-categorize associated bills
    const billReq = billStore.openCursor();
    billReq.onsuccess = (e) => {
      const cursor = e.target.result;
      if (cursor) {
        const bill = cursor.value;
        if (bill.categoryId === Number(id)) {
